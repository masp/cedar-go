...
func (p *Parser) parseSchema() *ast.Schema {
	...
	if len(comments) > 0 {
		schema.FooterComments = comments
	}
	return schema
}
...
func (p *Parser) parseAction() (action *ast.Action) {
	...
	semi, _ := p.eatOnly(token.SEMICOLON, "expected semicolon")
...
func (p *Parser) parseTypeDecl() (typ *ast.CommonTypeDecl) {
	...
	p.eatOnly(token.TYPE, "expected type keyword")
...
func (p *Parser) parseType() (typ ast.Type) {
	if p.matches(token.LEFTBRACE) {
		...
		typ = p.parsePath()
	} else if p.matches(token.BOOL_TYPE, token.LONG_TYPE, token.STRING_TYPE) {
		t := p.eat()
		typ = &ast.Path{Parts: []*ast.Ident{{Value: t.Lit, IdentTok: t.Pos}}}
	} else if p.matches(token.SET_TYPE) {
		setTok := p.eat()
