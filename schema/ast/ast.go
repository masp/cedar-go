...
func (s *Schema) End() token.Position {
	...
	if len(s.FooterComments) > 0 {
		return s.FooterComments.End()
	}
	return token.Position{}
}
...
func (n *Namespace) Pos() token.Position {
	if n.NodeComments.Before != nil {
		return n.NodeComments.Before[0].SlashTok
	}
...
func (p *Path) End() token.Position {
	...
	return p.Parts[len(p.Parts)-1].IdentTok
}
...
func (c CommentBlock) Pos() token.Position {
	if len(c) == 0 {
		panic("CommentBlock cannot be empty")
	}
	return c[0].SlashTok
}
